...
class NewsItem(
    ...
    val newsText = child<KNode> {
        hasTestTag(NEWS_TEXT)
    }

    val newsImage = child<KNode> {
        hasTestTag(NEWS_IMAGE)
    }

    val metadataText = child<KNode> {
        hasTestTag(METADATA_TEXT)
    }

    val rowTestTag = child<KNode> {
        hasTestTag(ROW_TEST_TAG)
    }
}